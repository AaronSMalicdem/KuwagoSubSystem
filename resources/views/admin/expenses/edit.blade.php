                    ...
                    <form method="POST" action="{{ route('admin.expenses.update', $expense->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="expense_type" id="expense_type" class="form-select w-full border border-gray-300 rounded-md p-2">
                                <option value="Food" {{ $expense->expense_type == 'Food' ? 'selected' : '' }}>Food/Beverage</option>
                                <option value="DispoDish" {{ $expense->expense_type == 'DispoDish' ? 'selected' : '' }}>Disposable Dish</option>
                                <option value="Cleaning" {{ $expense->expense_type == 'Cleaning' ? 'selected' : '' }}>Cleaning</option>
                                <option value="Equipment" {{ $expense->expense_type == 'Equipment' ? 'selected' : '' }}>Equipment/Maintenance</option>
                                <option value="OfficeSupply" {{ $expense->expense_type == 'OfficeSupply' ? 'selected' : '' }}>Office Supply</option>
                                <option value="Decor" {{ $expense->expense_type == 'Decor' ? 'selected' : '' }}>Market/Decor</option>
